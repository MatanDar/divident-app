...
function HushlimuContent() {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await fetch(`/api/hushlimu/${id}`, { method: 'DELETE' });
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-screen-xl mx-auto px-4 py-6 pb-24">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <div>
            <h1 className="text-2xl font-bold text-sky-800">הושלמו</h1>
          ...
          <div className="flex gap-2">
            ...
            <Link href="/hushlimu/new" className="inline-flex items-center gap-2 bg-sky-600 hover:bg-sky-500 text-white px-5 py-2.5 rounded-lg font-medium text-sm transition-colors shadow-sm">
              + קריאה חדשה
            </Link>
          </div>
        </div>
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <div className="flex gap-1 justify-end">
                            <Link href={`/hushlimu/call/${call.id}`}
...
export default function HushlimuPage() {
  return (
    <Suspense>
      <HushlimuContent />
    </Suspense>
